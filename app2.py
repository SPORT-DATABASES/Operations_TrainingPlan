...
def convert_to_time(timestamp_ms, offset_hours=10):
    if isinstance(timestamp_ms, (pd.Series, list, tuple)):
        # Handle iterable case using list comprehension
        return [(datetime.fromtimestamp(float(x) / 1000, tz=timezone.utc) - timedelta(hours=offset_hours)).strftime('%H:%M')
                if pd.notnull(x) else None for x in timestamp_ms]
...
def generate_excel(selected_date):
    ...
    df = df.query("Sport.notna() and Sport.str.strip() != '' and Venue != 'AASMC' and Sport != 'Generic_Athlete' and Training_Group != 'Practice'")
    ...
    for day_offset, cell_group in enumerate(zip(*date_cells_groups)):
        ...
        formatted_dates[day_offset] = date_value
...
def generate_venue_usage_report(filtered_df, start_date):
    ...
    for i in range(0, len(venues), page_capacity):
        ...
        for venue in venue_subset:
            ...
            # Cache formatted dates and string conversions
            formatted_dates = {}
            for _, row in venue_data.iterrows():
                date_obj = row['Date']
                if date_obj not in formatted_dates:
                    formatted_dates[date_obj] = date_obj.strftime('%A %d %b %Y')
                date_str = formatted_dates[date_obj]
...
min_date = datetime(2025, 1, 5).date()  # First Sunday in 2025
max_date = datetime(2025, 12, 28).date()  # Last Sunday in 2025
...
selected_date = st.date_input(
    "Select a starting date (Sundays only)",
    value=next_sunday,
    min_value=min_date,
    max_value=max_date,
    disabled=False,
...
if st.session_state.generated:
    ...
    st.dataframe(st.session_state.pivot_df.style.highlight_null())
    st.download_button(
